...
class ChatServerTest {
    ...
    void setUp() {
        ...
        user1 = new User("Paul", server); // Assuming user1 is correctly registered in the server setup.
        user2 = new User("Michael", server);
        System.setOut(new PrintStream(outContent)); // Redirects System.out output.
    }
    ...
    void testBlockedUserCannotSendMessage() {
        // Clear the output buffer to ensure only relevant output is captured after this point.
        outContent.reset();

        // Setup the block and send the message.
        server.blockMessage("Michael", "Paul");
        server.sendMessage("Paul", Arrays.asList("Michael"), "Can you hear me, Michael?");

        // Expected output should only include the result of the blocked message attempt.
