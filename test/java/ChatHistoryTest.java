...
public class ChatHistoryTest {
    ...
    public void testRemoveMessage() {
        ...
        assertFalse(chatHistory.removeMessage(lastMessage)); // Try to remove again
    }
    ...
    public void testIterator() {
        ...
        assertEquals("user1 to user2: Hello user2!", iterator.next().getContent());  // Correcting the expected content
        assertTrue(iterator.hasNext());  // Checking if there's another message
        assertEquals("user2 to user1: Hello user1!", iterator.next().getContent());  // Expecting the second message
        assertFalse(iterator.hasNext());  // No more messages expected
    }
